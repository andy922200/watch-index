...
- 目前正式資料涵蓋的品牌與市場見下表；筆數以 [README.md](README.md) 與實際資料檔為準。
...
| 市場 `marketCode` | 勞力士 `rolex` | Omega `omega` | Longines `longines` |
| --- | :---: | :---: | :---: |
| 台灣 `TW` | ✓ | ✓ | ✓ |
| 中國 `CN` | ✓ | ✓ | |
| 香港 `HK` | ✓ | ✓ | |
...
「不可變更」自 evidence 進入版本控制（commit）時起算。同一輪收集尚未 commit 前，若因逾時、防護阻擋、分頁中斷或補抓欄位而多次嘗試，可將各次嘗試整併為單一 run 的 evidence，條件如下：

- 僅限同一品牌、同一市場、同一輪收集且尚未 commit 的檔案；已 commit 的 evidence 一律依前述規則處理，只能另立更正記錄。
- 在 `collection-summary.json` 保留每次嘗試的時間、方法、範圍、結果與錯誤摘要，失敗或中斷的嘗試不得因整併而消失。
- 觀察值逐筆保留來源 URL 與實際觀察時間；不同嘗試對同一配置取得不一致的價格或狀態時，兩筆都要保留並記錄衝突，不得擇一丟棄。
- 整併只能重組已實際觀察到的內容，不得補造未觀察的欄位，也不得在整併時把機敏資料帶進檔案。
- 整併完成後才以該 run 更新正式資料與 history；history 的 `runId` 必須指向整併後的 evidence。
...
- [ ] 未修改、刪除或壓縮已 commit 的 evidence；本次結果與先前不一致時保留原記錄，必要時另建更正記錄。
- [ ] 若整併了本輪尚未 commit 的多次嘗試，已在 `collection-summary.json` 保留每次嘗試的紀錄與衝突，未丟棄或補造觀察。
