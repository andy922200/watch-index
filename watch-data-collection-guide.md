...
資料來源以目標市場的品牌官方公開資訊為準，記錄官方在售配置、當地文字、官方商品連結與建議零售價。目前正式資料包含勞力士的 15 個市場，以及 Omega Taiwan 的完整官方目錄；本指南的欄位與契約適用於所有品牌，既有範例若使用 Rolex 檔名僅為說明。它不收錄二手或平行市場價格、授權經銷商庫存，也不以其他市場價格或匯率推算缺漏值。
...
勞力士目前已收錄 15 個市場，各市場皆有 1,465 筆配置資料：
...
Omega Taiwan 已於 2026-09-18 自 `omegawatches.com.tw` 的 16 個官方分類完整收集 551 個唯一參考編號，語系為 `zh-Hant-TW`、幣別為 TWD。商品頁未說明價格稅別，故依既有台灣市場規則記為含稅 5%；這不是 Omega 官網對稅別的明示。完整分頁停止證據、原始價格與此限制保存在 `data/evidence/omega/TW/2026-09-18/`；俗稱研究尚未執行，相關欄位保留空陣列而非宣稱不存在。
